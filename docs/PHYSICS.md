...
| Half-open `[start, end)` endpoint rule | ✅ `apsis.test.ts`, `shell.test.ts` — two abutting searches report every crossing exactly once, which is the property a timeline of touching arcs depends on |
