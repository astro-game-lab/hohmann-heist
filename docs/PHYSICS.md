...
- **The benchmarks are gated against a committed baseline, not only against a hard limit.** An absolute limit catches a catastrophe and nothing else: a change that costs forty percent breaks none of them, and neither does the next one. `tools/bench/compare.mjs` compares every measurement against `tools/bench/baseline.json` and fails on a regression. Two choices in it were measured rather than assumed, and both came out slightly against expectation. The comparison uses the **minimum** over the batches, not the median: timing noise is one-sided, so the minimum estimates the cost and the median estimates the cost plus that run's interference — over seven consecutive runs of the whole suite the typical spread is the same either way (12.2% against 11.8%) but the *worst row* is 19.7% against 31.5%, and a threshold is set by the worst row. And every measurement is divided by a frozen scalar arithmetic yardstick measured in the same process, which does **not** make the comparison quieter on one machine — it moves the worst row from 19.7% to 21.2%, because the yardstick carries about 10% run-to-run range of its own — but is what lets a baseline recorded on one machine gate a run on another, which the gate cannot work without. The tolerance is 50%, a little over twice the worst spread observed, and the reasoning is written where the number is so it cannot be widened without an argument.
