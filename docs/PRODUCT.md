...
| **Environments** | `production` → GitHub Pages at the repo's Pages URL, served from the root of the `gh-pages` branch (custom domain optional later); `preview` → the same Pages site under `pr-preview/pr-<n>/`, one per open pull request, built from the PR's head and removed when it closes; `local` → `pnpm dev`. Pages has no native per-PR deployment — `actions/deploy-pages`' `preview` input is alpha and not public, and a repository gets one Pages site — so previews and production coexist as paths in a single published tree: a root publish deletes everything it replaces *except* `pr-preview/`, and a preview writes nowhere but its own directory. A pull request from a fork gets no preview, because a fork's token is read-only and `pull_request_target` would run fork code with a writable one; that job reports the skip rather than failing. The API has `production` and `staging` Workers. |
...
12. `infra: PR preview deployments` — M
