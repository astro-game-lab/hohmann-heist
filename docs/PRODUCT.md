...
| D9 | **Preact** for the UI shell. *(Amended at M2 exit: was "Preact + `@preact/signals`".)* | React-shaped API most contributors already know, at ~4 kB. React remains a drop-in via `preact/compat` if this proves wrong. **Decided at M2 exit: go on Preact, and `@preact/signals` dropped** — the measurement (#248) put HUD re-render at 0.285 ms/frame at 1× and 0.420 ms at 10 000×, against R11's 2 ms trigger, with zero frames dropped; and signals turned out to be a declared dependency that nothing imported and the bundle never contained. The claim they were carried for — that fine-grained subscription is needed to avoid re-rendering panels at 60 Hz — is false here: re-rendering the whole subtree costs a fifth of a millisecond. See `docs/DECISION-D9.md`. | Low — and the removal is reversible in an afternoon. |
...
| R11 | **Preact/signals turns out to be the wrong shell** for a 60 Hz HUD. | Low | Moderate | Decision D9 is explicitly reviewable at M2 exit; `preact/compat` makes React a drop-in. **Measured at M2 exit (#248): 0.285–0.420 ms median, 1.30 ms worst — the trigger did not fire and the risk is retired for the shell.** It stays live for the *tail*: the worst frame is 65% of the trigger, and M3 stacks the board, settings and medals on top. | HUD re-render cost above 2 ms/frame → switch. |
...
| A3 | Analytic Kepler propagation is fast enough that no numerical integration is needed in the game path (D5). | **Held** — M1's spike (#238) measured it at 0.27% of a 60 Hz frame in a browser. The spike falsified it early; the fallback is a fixed-step integrator with a state cache, which costs the "scrub anywhere instantly" property. |
