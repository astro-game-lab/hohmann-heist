...
**Status: M3 of eight — the alpha, released as `v0.2.0`.** Acts I and II are playable: seven contracts, each running briefing → planner → execution → debrief, reached from a title screen and a contract board with medals, credits and act unlocks. `@hh/math`, `@hh/astro`, `@hh/propagation` and `@hh/render` hold real code; `@hh/sim` holds the plan and its evaluation — nodes, quantisation, impulsive Δv, replay codes, and the timeline that turns a plan into a trajectory; `@hh/game` holds objectives, constraints, legality, medals, progression, the scenario format and every DEP-xx; `@hh/ui` holds the message catalogue, §8.5.1's state machine, the palettes, the Codex entries and the coach-mark table; `apps/web` holds the routing shell and every screen. `README.md` describes what exists at any given time and is kept honest — trust it over any assumption about what has landed.

Two of M3's exit criteria were **not** met at the tag and are open: §13's eight playtest sessions (#209, plus M2's deferred round #208) and the public alpha announcement (#278). `CHANGELOG.md`'s `v0.2.0` entry records that, and §14.1's M2 note is explicit that M3's criteria are *"not waivable by this precedent"* — so this was a deliberate call rather than a precedent to follow again. Do not describe the alpha as playtested.
