...
import type { Camera, HitIndex, MarkerSpec, NodeSpec, ViewBounds } from '@hh/render';
import {
  EQUATORIAL_BASIS,
  MAX_ZOOM,
  MIN_ZOOM,
...
export const OrbitView = ({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent): void => {
      ...
      pressed = { x: event.clientX, y: event.clientY, moved: true };
      const framing = framingRef.current;
      if (framing === null) return;
    ...
    const onPointerUp = (event: PointerEvent): void => {
      ...
      const hit = hitTest(index, localPoint(event));
      // Handles and the trajectory are #133–#135's; in this pass a click selects a node
      // or clears the selection, which is §8.5.2's first and third rows.
      ...
      <div class="hh-orbit__controls">
        <button
          ...
          onClick={() => {
            ...
            // which re-installs `requestRef` and calls it — the effect owns the camera,
